...
async def run():
    async with async_playwright() as p:
        ...
        # Search across dates from 2025 to 2026
        for year in [2025, 2026]:
            for m in range(0, 12):
                for d in range(1, 32):
                    ...
                    # Standardize स्वाति / स्वाती key
                    key_name = "स्वाती" if nak_name == "स्वाति" else nak_name

                    if key_name and key_name not in results:
                        exp_y = EXPECTED_YONI.get(key_name, "")
                        exp_v = EXPECTED_VRUKSHA.get(key_name, "")
                        exp_a = EXPECTED_AKSHAR.get(key_name, "")
